 ...
 * Date: 25.01.12
 */
public class NapileRefactoringUtil
{

	private NapileRefactoringUtil()
	{
	}
	...
	{
		if(offset < 0)
			throw new IntroduceRefactoringException(NapileRefactoringBundle.message("cannot.refactor.not.expression"));
				...
				!(element instanceof NapileNamedMethodOrMacro) && !(element instanceof NapileClassBody))
		{
			if(element instanceof NapileExpression && !(element instanceof NapileStatementExpression))
			{
				...
				if(addExpression)
				{
					...
					NapileType expressionType = bindingContext.get(BindingTraceKeys.EXPRESSION_TYPE, expression);
		...
		JBPopupFactory.getInstance().createListPopupBuilder(list).
				setTitle(NapileRefactoringBundle.message("expressions.title")).setMovable(false).setResizable(false).
