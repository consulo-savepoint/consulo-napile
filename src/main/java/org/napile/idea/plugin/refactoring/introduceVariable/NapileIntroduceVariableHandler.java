 ...
 * Date: 25.01.12
 */
public class NapileIntroduceVariableHandler extends NapileIntroduceHandlerBase
{

	private static final String INTRODUCE_VARIABLE = NapileRefactoringBundle.message("introduce.variable");

	@Override
	public void invoke(@NotNull final Project project, final Editor editor, final PsiFile file, DataContext dataContext)
	{
		NapileRefactoringUtil.SelectExpressionCallback callback = new NapileRefactoringUtil.SelectExpressionCallback()
		{
			@Override
		...
		try
		{
			NapileRefactoringUtil.selectExpression(editor, file, callback);
		}
		catch(NapileRefactoringUtil.IntroduceRefactoringException e)
		{
	...
	private static void doRefactoring(@NotNull final Project project, final Editor editor, @Nullable NapileExpression _expression)
	{
		if(_expression == null)
		{
			showErrorHint(project, editor, NapileRefactoringBundle.message("cannot.refactor.no.expression"));
			return;
		}
		...
		final NapileType expressionType = bindingContext.get(BindingTraceKeys.EXPRESSION_TYPE, expression); //can be null or error type
		NapileScope scope = bindingContext.get(BindingTraceKeys.RESOLUTION_SCOPE, expression);
		if(scope != null)
		{
			...
			NapileType typeNoExpectedType = injector.getExpressionTypingServices().getType(scope, expression, TypeUtils.NO_EXPECTED_TYPE, dataFlowInfo, injector.getBindingTrace());
			if(expressionType != null && typeNoExpectedType != null && !NapileTypeChecker.INSTANCE.equalTypes(expressionType, typeNoExpectedType))
			{
		...
		{
			showErrorHint(project, editor, NapileRefactoringBundle.message("cannot.refactor.namespace.expression"));
			return;
		}
		if(expressionType == null && noTypeInference)
		{
			showErrorHint(project, editor, NapileRefactoringBundle.message("cannot.refactor.expression.should.have.inferred.type"));
			return;
		}
		...
		if(container == null)
		{
			showErrorHint(project, editor, NapileRefactoringBundle.message("cannot.refactor.no.container"));
			return;
		}
		...
		Pass<OccurrencesChooser.ReplaceChoice> callback = new Pass<OccurrencesChooser.ReplaceChoice>()
		{
			...
			public void pass(OccurrencesChooser.ReplaceChoice replaceChoice)
			{
				...
				NapileNameValidatorImpl validator = new NapileNameValidatorImpl(commonContainer, calculateAnchor(commonParent, commonContainer, allReplaces));
				String[] suggestedNames = NapileNameSuggester.suggestNames(expression, validator);
					...
					public void run()
					{
						...
						if(property != null)
						{
							...
							if(isInplaceAvailableOnDataContext)
							{
								...
								NapileInplaceVariableIntroducer variableIntroducer = new NapileInplaceVariableIntroducer(property, editor, project, INTRODUCE_VARIABLE, references.toArray(new NapileExpression[references.size()]), reference.get(), finalReplaceOccurrence, property, /*todo*/false, /*todo*/false, expressionType, finalNoTypeInference);
